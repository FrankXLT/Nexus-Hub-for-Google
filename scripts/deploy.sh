...
if command -v clasp &> /dev/null; then
    ...
    if [ -n "$DEPLOYMENT_ID" ]; then
        read -p "Found existing Apps Script deployment ($DEPLOYMENT_ID). Do you want to [U]pdate this deployment, or create a [N]ew one? (U/n): " DEPLOY_ACTION
        if [ -z "$DEPLOY_ACTION" ]; then
            DEPLOY_ACTION="U"
        fi
    fi

    if [[ "$DEPLOY_ACTION" =~ ^[Uu]$ ]]; then
        DEPLOY_OUT=$(clasp deploy -i $DEPLOYMENT_ID -d "Nexus Auto-Deploy $(date +'%Y-%m-%d %H:%M')" 2>&1)
        ...
        if grep -q "^DEPLOYMENT_ID=" .nexus_env; then
            tmp_file=$(mktemp)
            sed "s/^DEPLOYMENT_ID=.*/DEPLOYMENT_ID=$DEPLOY_ID/" .nexus_env > "$tmp_file"
            mv "$tmp_file" .nexus_env
        else
            echo "DEPLOYMENT_ID=$DEPLOY_ID" >> .nexus_env
        fi
    fi
...
echo -e "\nPlease copy the variables above. In a moment, your browser will open the Apps Script Editor. You MUST immediately go to: Project Settings (Gear Icon) -> Script Properties -> Add Script Property. Add NEXUS_HMAC_SECRET and NEXUS_VM_URL."
echo ""
