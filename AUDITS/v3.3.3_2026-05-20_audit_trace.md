...
sequenceDiagram
    ...
    User->>Frontend: Trigger Sync
    Frontend->>SyncEngine: Call sync_drive
    SyncEngine->>SQLite: Query Sync_State
    SyncEngine->>SyncEngine: Fetch Drive Files
    SyncEngine->>LLM: process_drive_document (Stage 1)
    SyncEngine->>LLM: process_drive_document (Stage 2)
    LLM->>SQLite: persist_llm_results
    SQLite-->>SyncEngine: Confirm
    SyncEngine-->>Frontend: Sync Complete
```
...
| Component | Assumption | Watch-out Flag |
| :--- | :--- | :--- |
| Drive API | Folder structure exists | ID resolution failures |
| OCR | Text is readable | Scanning artifacts |
| LLM | JSON output is valid | Hallucinated JSON format |

### Pipeline: Contacts Ingestion
```mermaid
sequenceDiagram
    participant User
    participant Frontend
    ...
    User->>Frontend: Trigger Contact Sync
    Frontend->>SyncEngine: Call sync_contacts
    SyncEngine->>SQLite: Query Sync_State
    SyncEngine->>SyncEngine: Fetch Contacts (People API)
    SyncEngine->>SQLite: Upsert entities/aliases
    SQLite-->>SyncEngine: Confirm
    SyncEngine-->>Frontend: Sync Complete
```
...
| Component | Assumption | Watch-out Flag |
| :--- | :--- | :--- |
| People API | Contact data is populated | Empty contact fields |
| SQLite | Transactional integrity | Lock contention on large upserts |
...
sequenceDiagram
    ...
    User->>Frontend: Trigger Bulk/Batch Process
    Frontend->>LLM: run_bulk_classifier
    LLM->>SQLite: Update Workspace_Artifacts
    SQLite-->>Frontend: UI Update
```

**Vulnerability & Assumption Matrix:**
| Component | Assumption | Watch-out Flag |
| :--- | :--- | :--- |
| LLM | Batch JSON is valid | Token limits exceeded |
| SQLite | Indexed performance | Concurrent write operations |

### Pipeline: Legacy Ingest
```mermaid
sequenceDiagram
    participant User
    participant Frontend
    ...
    User->>Frontend: Trigger Legacy Import
    Frontend->>SyncEngine: queueHistoricalImport
    SyncEngine->>SQLite: Stage in Legacy_Label_Migration
    SyncEngine->>LLM: run_bulk_legacy_mapper
    LLM->>SQLite: Update Workspace_Artifacts & History
    SQLite-->>Frontend: UI Update
```
...
| Component | Assumption | Watch-out Flag |
| :--- | :--- | :--- |
| Legacy Labels | Labels are well-formed | Duplicate label name collisions |
| LLM | Confidence scores are calibrated | High quarantine rate on vague labels |
| SQLite | Schema migration safety | Migration transaction failure |
